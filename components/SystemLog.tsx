...
export function SystemLog({ logs, maxDisplayed = 50 }: SystemLogProps) {
    const bottomRef = useRef<HTMLDivElement>(null);
    const displayed = logs.slice(-maxDisplayed);

    // Auto-scroll ke bawah ketika ada log baru
    useEffect(() => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <span className="text-gray-700 leading-5 break-all">
                                        {log.message}
                                        {log.details && (
                                            <span className="ml-1 text-gray-400">— {log.details}</span>
                                        )}
                                    </span>
                                </div>
                            );
